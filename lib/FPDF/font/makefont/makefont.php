...
function MakeFont($fontfile,$afmfile,$enc='cp1252',$patch=array(),$type='TrueType')
{
	...
	if($fontfile)
	{
		...
		if($type=='Type1')
		{
			...
			if(!$pos)
				die("Error: font file does not seem to be valid Type1n\n");
			$size1=$pos+6;
...
if($argc < 2) {
	echo "Usage: makefont.php ttf-file [ttf-file] [...]\n";
	exit;
}
...
foreach($ttf_files as $ttf_file) {
	...
	if(substr($ttf_file, strrpos($ttf_file, '.')) != '.ttf') {
		echo "Skipping $ttf_file \n";
		continue;
	}
	
	$name = basename($ttf_file, '.ttf');
	
	system("ttf2pt1 $ttf_file");
	MakeFont($ttf_file, 	$name . '.afm');
	rename($name . '.php', 	sprintf($install_target, $name . '.php'));
	rename($name . '.z',		sprintf($install_target, $name . '.z'));
	echo "Completed $name \n";
}
